...
export class WebRTCConnection extends BaseConnection {
  ...
  public static async create(
    ...
    try {
      ...
      // Observe connect independently of microphone settlement so a hung
      // microphone cannot own the late-room lifecycle.
      const lateRoomCleanup = roomConnected.then(
        async () => {
      ...
      registerSessionCleanup(config.signal, lateRoomCleanup);
      registerSessionCleanup(config.signal, micEnabled);
      ...
      if (connection) {
        ...
        // Only a cancelled start rejects before teardown settles; close()
        // registers it as session cleanup. Any other failure waits, because
        // callers without that barrier would otherwise retry onto live
        // resources.
