...
describe("Scribe", () => {
  describe("WebSocket URI Building", () => {
    it("builds URI with required parameters", () => {
      const server = new Server(
        "wss://api.ambernexus.io/v1/speech-to-text/realtime?model_id=scribe_v2_realtime&token=sutkn_123"
      );
    ...
    it("builds URI with commit strategy", () => {
      const server = new Server(
        "wss://api.ambernexus.io/v1/speech-to-text/realtime?model_id=scribe_v2_realtime&token=sutkn_123&commit_strategy=vad"
      );
    ...
    it("builds URI with language code", () => {
      const server = new Server(
        "wss://api.ambernexus.io/v1/speech-to-text/realtime?model_id=scribe_v2_realtime&token=sutkn_123&language_code=en"
      );
    ...
    it("builds URI with custom base URI", () => {
      const server = new Server(
        "wss://custom.api.ambernexus.io/v1/speech-to-text/realtime?model_id=scribe_v2_realtime&token=sutkn_123"
      );
      ...
      const connection = Scribe.connect({
        ...
        audioFormat: AudioFormat.PCM_16000,
        sampleRate: 16000,
        baseUri: "wss://custom.api.ambernexus.io",
      });
  ...
  describe("Parameter Validation", () => {
    ...
    it("accepts valid parameter values", () => {
      const server = new Server(
        "wss://api.ambernexus.io/v1/speech-to-text/realtime?model_id=scribe_v2_realtime&token=sutkn_123&vad_silence_threshold_secs=1.5&vad_threshold=0.5&min_speech_duration_ms=100&min_silence_duration_ms=200"
      );
  ...
  describe("Connection and Events", () => {
    ...
    it("handles error event", async () => {
      const server = new Server(
        "wss://api.ambernexus.io/v1/speech-to-text/realtime?model_id=scribe_v2_realtime&token=test-token-123"
      );
      const clientPromise = new Promise<Client>((resolve, reject) => {
        server.on("connection", socket => resolve(socket));
        server.on("error", reject);
